...
export default class Order extends Component {
  ...
  repeatOrder = async (order) => {
    try {
      // Сохраняем данные заказа для повторения
      await AsyncStorage.setItem('repeat_order', JSON.stringify(order));
      // Переходим на экран ресторана
                                  ...
                                  <TouchableOpacity key={i} onPress={() => this.props.navigation.navigate('Orderview',{id:order.id})}>
                                      <View style={styles.singleOrder}>
                                          ...
                                          <View style={styles.orderActions}>
                                              <TouchableOpacity style={styles.viewOrderBtn} onPress={() => this.props.navigation.navigate('Orderview',{id:order.id})}>
                                                  <Feather name="eye" size={24} color="#fff" />
                                              </TouchableOpacity>
                                              <TouchableOpacity style={styles.repeatOrderBtn} onPress={() => this.repeatOrder(order)}>
