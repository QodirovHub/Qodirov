    ...
    async componentDidMount() {
        ...
        return fetch(config.APP_URL + '/api/restaurant/'+id+'', {
            ...
            .then((json) => {
                ...
                // Проверяем, есть ли заказ для повторения
                const repeatOrder = await AsyncStorage.getItem('repeat_order');
                if (repeatOrder) {
                    this.setState({ repeatOrder: JSON.parse(repeatOrder) });
                    // Очищаем сохраненный заказ
                    await AsyncStorage.removeItem('repeat_order');
                }
    ...
    render() {
        ...
        if (this.state.isLoading) {
            ...
            if (this.state.error) {
                ...
                if (this.state.repeatOrder) {
                    // Если есть заказ для повторения, добавляем товары в корзину
                    this.state.repeatOrder.items.forEach(item => {
                        this.addToCart(item);
                    });
                    // Очищаем состояние повторения
                    this.setState({ repeatOrder: null });
                }
                return (
                    <View style={styles.flex}>
